---
source_url: https://claude.com/resources/tutorials/imagine-with-claude-student-guide
...
- Share [Copy link](https://claude.com/resources/tutorials/imagine-with-claude-student-guide#)https://claude.com/resources/tutorials/imagine-with-claude-student-guide
...
<!-- para:0014 -->
Watch it build, one morpheme at a time. The root is *gör*, "see":
...
<!-- para:0018 -->
Short answer: **light adds, paint subtracts.** A screen stacks light on light — more mixing means more energy hitting your eye, trending toward white. Paint is a *filter* — each pigment steals wavelengths, and stacking filters means *less* light surviving to reach your eye.
...
<!-- para:0025 -->
You've hit on exactly why linguists reach for logic notation — English word order flattens something that's actually *nested*. The divergence lives in which quantifier wraps around which.

<!-- para:0026 -->
"Every" and "a" are both quantifiers, and quantifiers *nest*. The sentence gives you the pieces but doesn't tell you which one wraps the other. That's the fork:

<!-- para:0027 -->
The real divergence is in **which quantifier gets to pick first**. ∀∃ hands each student the existential separately — nothing forces them to agree. ∃∀ picks the book *once*, up front, and that single choice has to work for everyone. English defaults to the weak one and lets context drag the strong one out when it's needed.
...
- *"The textbook explains this in words. I need the diagram it didn't include."*
...
- *"I know something's wrong in here: [upload]. Walk me through where I went off."*
