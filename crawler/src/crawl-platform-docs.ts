...
import { discoverPlatformDocsUrls, extractPlatformDocsPage } from "./lib/platform-docs.js";
...
async function main(): Promise<void> {
  ...
  const urls = await discoverPlatformDocsUrls(options.limit);
  console.log(`Discovered ${urls.length} platform docs URLs`);
  ...
  await runWithConcurrency(urls, options.concurrency, async (url, index) => {
    try {
      ...
      const page = extractPlatformDocsPage(url, html);
...
async function writeRunArtifacts(payload: {
  ...
  const reportPath = path.join(outputDir, `platform-docs-run-${timestamp}.json`);
  const failurePath = path.join(outputDir, "platform-docs-failures.json");
