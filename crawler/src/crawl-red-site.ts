...
const SITE = {
  homeUrl: "https://red.anthropic.com/",
  section: "blog",
  siteTitle: "red.anthropic.com",
} as const;

async function main(): Promise<void> {
  const options = parseArgs(process.argv.slice(2));
  const urls = await discoverDistillArticleUrls(SITE, options.limit);
  console.log(`Discovered ${urls.length} red URLs`);
...
async function writeRunArtifacts(payload: {
  ...
  const reportPath = path.join(outputDir, `red-site-run-${timestamp}.json`);
  const failurePath = path.join(outputDir, "red-site-failures.json");
